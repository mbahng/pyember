...
Your machine will need system dependencies such as CMake, a C++ compiler, and pybind11. Preferably you will have git and conda installed already. For more specific instructions on installing these on your system, refer to the more detailed installation guide. 
...
This runs `cmake` on `aten/CMakeLists.txt`, which calls `aten/CMakeLists.txt` that compiles and links the source files in the C++ tensor library, which in turn (by default argument `BUILD_PYTHON_BINDINGS=ON`) calls `aten/bindings/CMakeLists.txt` to further generate a `.so` file that can be imported into `ember`. If there are problems, you should first check 
...
## Repository Structure 
  ...
  3. `ember` contains the actual library, supporting high level models, objectives, optimizers, dataloaders, and samplers. 
