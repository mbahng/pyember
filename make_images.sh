...
#
# debian x86
docker buildx build --platform linux/amd64  --build-arg PYTHON_VERSION=3.12 --build-arg OS_VERSION=12 -t pyember-debian.12-amd -f docker/debian_amd.Dockerfile --load --no-cache .
docker buildx build --platform linux/amd64  --build-arg PYTHON_VERSION=3.12 --build-arg OS_VERSION=11 -t pyember-debian.11-amd -f docker/debian_amd.Dockerfile --load --no-cache .
docker buildx build --platform linux/amd64  --build-arg PYTHON_VERSION=3.12 --build-arg OS_VERSION=10 -t pyember-debian.10-amd -f docker/debian_amd.Dockerfile --load --no-cache .
